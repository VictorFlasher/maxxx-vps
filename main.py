...
# Создаем кастомное окружение Jinja2 с отключенным кэшем
from jinja2 import Environment, FileSystemLoader, select_autoescape
...
async def chat_page(request: Request, chat_id: int = None):
    ...
    return templates.TemplateResponse(request, "chat.html", {"request": request, "chat_id": chat_id})
...
async def admin_panel_redirect(request: Request):
    ...
    return templates.TemplateResponse(request, "admin.html", {"request": request, "user_id": None})


@app.get("/search-users")
async def search_users_page(request: Request):
    """Страница поиска пользователей."""
    return templates.TemplateResponse(request, "search_users.html", {"request": request})
